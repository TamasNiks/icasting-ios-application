    ...
    func setStatus(status: FilterStatusFields?) {
        ...
        if let status = status {
            ...
            if let color = MatchStatusColor.color(status) {
                ...
                let statusIndicator = StatusIndicator(frame: CGRectMake(0, 0, 11, 11), color: color)
        ...
        let statusIndicator = StatusIndicator(frame: CGRectMake(0, 0, 15, 15), color: UIColor.grayColor())
        statusIndicator.tag = tagForReset
    }
